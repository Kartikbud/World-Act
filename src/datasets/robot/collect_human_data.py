...
def review_video(frames):
    ...
    try:
        ...
        pass
    while True:
        ans = input("Approve this demo? [y/n]: ").strip().lower()
        if ans in ("y", "n"):
            return ans == "y"
...
def main():
    parser = argparse.ArgumentParser(description="Collect Stack teleop demos")
    parser.add_argument(
        "num_episodes",
        nargs="?",
        type=int,
    ...
    print(f"Keyboard teleop. Press q to abort an episode. Success must be < {MAX_STEPS} steps.")
