...
from architectures.robot_encoder import RobotEncoder
from architectures.predictor import PredictorNetwork
from datasets.robot.primary_dataset import RobotPrimaryDataset
...
def train(device,
    ...
    train_dataset = RobotPrimaryDataset(data_dir=data_dir,
                                 frame_skip=frame_skip,
                                 window=window_size,
                                 val=False)

    val_dataset = RobotPrimaryDataset(data_dir=data_dir,
                               frame_skip=frame_skip,
                               window=window_size,
                               val=True)
    ...
    encoder = RobotEncoder(patch=patch_size,
                             nheads=enc_nheads,
                             nlayers=enc_nlayers,
    ...
    for epoch in tqdm(range(epochs)):
        ...
        encoder.train()
        predictor.train()
        for batch, (above_in, wrist_in, state_in, actions, above_tgt, wrist_tgt, state_tgt) in enumerate(train_dataloader):
            above_in = above_in.to(device, non_blocking=non_blocking)
            wrist_in = wrist_in.to(device, non_blocking=non_blocking)
            state_in = state_in.to(device, non_blocking=non_blocking)
            actions = actions.to(device, non_blocking=non_blocking)
            above_tgt = above_tgt.to(device, non_blocking=non_blocking)
            wrist_tgt = wrist_tgt.to(device, non_blocking=non_blocking)
            state_tgt = state_tgt.to(device, non_blocking=non_blocking)

            with autocast_ctx:
                embeddings = encoder(above_in, wrist_in, state_in)
                pred_embeddings = predictor(embeddings, actions)
                target_embeddings = encoder(above_tgt, wrist_tgt, state_tgt)
        ...
        with torch.inference_mode():
            for (above_in, wrist_in, state_in, actions, above_tgt, wrist_tgt, state_tgt) in val_dataloader:
                above_in = above_in.to(device, non_blocking=non_blocking)
                wrist_in = wrist_in.to(device, non_blocking=non_blocking)
                state_in = state_in.to(device, non_blocking=non_blocking)
                actions = actions.to(device, non_blocking=non_blocking)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=PROJECT_DIR / "configs" / "default_primary.yaml",
